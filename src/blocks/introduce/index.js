...
export default function Introduce(props) {
  const content = props.content.lists.map((item) =>
    <div key={item.id} style={{ backgroundImage: `url(${require('../../assets/images/'+item.bg+'')})` }}
         className="md:py-10 py-4 lg:px-12 md:px-7 px-5 rounded-[10px] bg-gray-light2 bg-right-bottom bg-no-repeat md:bg-contain bg-[length:350px]">
      <div className="md:max-w-sm max-w-[220px]">
        <h2 className="md:text-4xl text-[21px] font-bold mb-2">{item.title}</h2>
        <p className="md:block hidden text-gray md:text-base text-sm">{item.description}</p>
  ...
  return (
    <section className="md:mb-[150px] mb-[70px]">
      <div className="container mx-auto grid gap-x-8 md:gap-y-0 gap-y-5 md:grid-cols-2">
        {content}
      </div>
    </section>
  )
